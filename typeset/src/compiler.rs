use bumpalo::Bump;
use std::{cell::Cell, cmp::max, fmt, option::Option};
...
*/
fn _broken<'b, 'a: 'b>(mem: &'b Bump, layout: Box<Layout>) -> &'b EDSL<'b> {
...
*/
fn _serialize<'b, 'a: 'b>(mem: &'b Bump, layout: &'a EDSL<'a>) -> &'b Serial<'b> {
    fn _next<'a>(
        mem: &'a Bump,
        term: &'a SerialTerm<'a>,
        comp: &'a SerialComp<'a>,
...
*/
fn _linearize<'b, 'a: 'b>(mem: &'b Bump, serial: &'a Serial<'a>) -> &'b LinearDoc<'b> {
...
*/
fn _fixed<'b, 'a: 'b>(mem: &'b Bump, doc: &'a LinearDoc<'a>) -> &'b FixedDoc<'b> {
...
fn _structurize<'b, 'a: 'b>(mem: &'b Bump, doc: &'a FixedDoc<'a>) -> &'b RebuildDoc<'b> {
...
*/
fn _denull<'b, 'a: 'b>(mem: &'b Bump, doc: &'a RebuildDoc<'a>) -> &'b DenullDoc<'b> {
...
*/
fn _identities<'b, 'a: 'b>(mem: &'b Bump, doc: &'a DenullDoc<'a>) -> &'b DenullDoc<'b> {
    fn _eod<'a>(mem: &'a Bump) -> &'a DenullDoc<'a> {
        mem.alloc(DenullDoc::EOD)
    }
    fn _empty<'a>(mem: &'a Bump, doc: &'a DenullDoc<'a>) -> &'a DenullDoc<'a> {
...
*/
fn _reassociate<'b, 'a: 'b>(mem: &'b Bump, doc: &'a DenullDoc<'a>) -> &'b DenullDoc<'b> {
    fn _eod<'a>(mem: &'a Bump) -> &'a DenullDoc<'a> {
        mem.alloc(DenullDoc::EOD)
    }
    fn _empty<'a>(mem: &'a Bump, doc: &'a DenullDoc<'a>) -> &'a DenullDoc<'a> {
...
*/
fn _rescope<'b, 'a: 'b>(mem: &'b Bump, doc: &'a DenullDoc<'a>) -> &'b FinalDoc<'b> {
...
impl fmt::Display for Doc {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        fn _print_obj(obj: Box<DocObj>) -> String {
            match obj {
                box DocObj::Text(data) => format!("(Text \"{}\")", data),
                box DocObj::Fix(obj1) => {
        ...
        fn _print_fix(obj: Box<DocObjFix>) -> String {
            match obj {
                box DocObjFix::Text(data) => format!("(Text \"{}\")", data),
...
fn _move_to_heap<'a>(doc: &'a FinalDoc<'a>) -> Box<Doc> {
...
pub fn render(doc: Box<Doc>, tab: usize, width: usize) -> String {
    ...
    fn _pad<'a>(n: usize, result: String) -> String {
        result + &_whitespace(n)
    }
    fn _measure<'b, 'a: 'b>(mem: &'b Bump, obj: &Box<DocObj>, state: State<'a>) -> usize {
        fn _visit_obj<'b, 'a: 'b>(mem: &'b Bump, obj: &Box<DocObj>, state: State<'a>) -> State<'b> {
            match obj {
        ...
        fn _visit_fix<'b, 'a: 'b>(fix: &Box<DocObjFix>, state: State<'a>) -> State<'a> {
            match fix {
                box DocObjFix::Text(data) => _inc_pos(data.len(), state),
                box DocObjFix::Comp(left, right, pad) => {
    ...
    fn _next_comp<'b, 'a: 'b>(mem: &'b Bump, obj: &Box<DocObj>, state: State<'a>) -> usize {
        fn _visit_obj<'b, 'a: 'b>(mem: &'b Bump, obj: &Box<DocObj>, state: State<'a>) -> State<'b> {
            match obj {
                box DocObj::Text(data) => _inc_pos(data.len(), state),
                box DocObj::Fix(fix) => _visit_fix(mem, fix, state),
                box DocObj::Grp(obj1) => {
                    let head = state.head;
                    if head {
                ...
                box DocObj::Comp(left, _right, _pad) => _visit_obj(mem, left, state),
            }
        }
        fn _visit_fix<'b, 'a: 'b>(
            mem: &'b Bump,
    ...
    fn _will_fit<'b, 'a: 'b>(mem: &'b Bump, obj: &Box<DocObj>, state: State) -> bool {
        let obj_end_pos = _measure(mem, obj, state);
        obj_end_pos <= state.width
    }
    fn _should_break<'b, 'a: 'b>(mem: &'b Bump, obj: &Box<DocObj>, state: State) -> bool {
    ...
    fn _visit_obj<'b, 'a: 'b>(
        ...
        match obj {
            ...
            box DocObj::Fix(fix) => _visit_fix(mem, fix, state, result),
            box DocObj::Grp(obj1) => {
                let broken = state.broken;
                let state1 = State {
                    broken: false,
