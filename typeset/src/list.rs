  ...
  pub fn map<S: Copy + Clone>(
    ...
    fn _visit<'b, 'a: 'b, A: Copy + Clone, B: Copy + Clone>(
      ...
      match items {
        ...
        List::Cons(_, item, items1) =>
          _visit(mem, items1, func, compose(mem, cont, mem.alloc(|mem, result|
          cons(mem, func(mem, *item), result))))
      }
    }
