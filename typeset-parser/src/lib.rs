...
use typeset::dsl::ParseError;
use typeset::dsl::grammar::{self, Binary, Build, Token, Unary};
...
fn tokenize(input: TokenStream) -> Result<Vec<(Span, Token<Ident>)>, Error> {
    ...
    while let Some((_, stream)) = streams.last_mut() {
        ...
        let token = match tree {
            ...
            TokenTree::Literal(literal) => match grammar::parse_text(&literal.to_string()) {
...
impl Build for Reify {
    ...
    fn line(&mut self, left: TokenStream, right: TokenStream) -> TokenStream {
        quote! { typeset::line(#left, #right) }
    }
    fn comp(&mut self, left: TokenStream, right: TokenStream, pad: Pad, brk: Break) -> TokenStream {
        let pad = match pad {
            Pad::Padded => quote! { typeset::Pad::Padded },
            Pad::Unpadded => quote! { typeset::Pad::Unpadded },
        };
        let brk = match brk {
            Break::Breakable => quote! { typeset::Break::Breakable },
            Break::Fixed => quote! { typeset::Break::Fixed },
        };
        quote! { typeset::comp(#left, #right, #pad, #brk) }
    }
}
...
fn expand(input: TokenStream) -> Result<TokenStream, Error> {
    ...
    grammar::parse_tokens(tokens, end, &mut Reify)
}
